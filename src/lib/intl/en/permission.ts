import { SystemLocaleMapping } from ".."

// 按照页面路由划分[pathname.var]:value
const enUS: SystemLocaleMapping = {
    "permission.btn.add": "Add permission",
    "permission.table.column.name": "Permission name",
    "permission.table.column.resource": "Resource name",
    "permission.table.column.type": "Resource type",
    "permission.modal.title.detail": "Permission detail",
    "permission.modal.title.add": "Add permission",
    "permission.modal.title.edit": "Edit permission",
    "permission.modal.label.name": "Permission name",
    "permission.modal.label.type": "Permission type",
    "permission.modal.placeholer.name": "Please enter a permission name",
    "permission.modal.placeholer.type": "Please enter a permission type",
    "permission.modal.label.resource": "Resource name",
    "permission.modal.placeholer.resource": "Please enter a resource name",
    "permission.modal.label.roles": "Roles",
    "permission.modal.placeholer.roles": "Please configure permission roles",
    "permission.modal.rule.required": "Required",
    "resource.type.view_route": "Frontend route",
    "resource.type.view_menu": "Frontend menu",
    "resource.type.view_resource": "Frontend resource",
    "resource.type.api_route": "Backend Api",
}

export default enUS
